...
```
                                  [ Client Request ]
                         (OpenAI SDK / cURL / Web Playground)
                                          │
                                          ▼
                      ┌────────────────────────────────────────┐
                      │    Token Pilot Gateway (NestJS :3000)  │
                      │  - Sliding Window Rate Limiting        │
                      │  - BYOK Key Format Validation          │
                      │  - Auth Guard & Input Sanitization     │
                      └───────────────────┬────────────────────┘
                                          │
                                          ▼
                      ┌────────────────────────────────────────┐
                      │   12-Signal Feature Extractor (0.6ms)  │
                      │  - Structural Depth & Graduated Code   │
                      │  - Reasoning vs Simplicity Keywords    │
                      │  - 40-Char Negation-Aware Scanning     │
                      │  - Domain Density & Academic Signals   │
                      └───────────────────┬────────────────────┘
                                          │
                                          ▼
                      ┌────────────────────────────────────────┐
                      │      Linear Scoring Engine (0.6ms)     │
                      │  - Normalized Weighted Sum [0.0 - 1.0] │
                      │  - Distance-to-Boundary Confidence     │
                      │  - Formal Language Density Multiplier  │
                      └───────────────────┬────────────────────┘
                                          │
           ┌──────────────────┬───────────┴──────────┬──────────────────┐
           │ Score <= 0.08    │ 0.08 < Score <= 0.20 │ 0.20 < Score <= 0.42 │ Score > 0.42
           ▼                  ▼                      ▼                  ▼
    ┌──────────────┐   ┌──────────────┐       ┌──────────────┐   ┌──────────────┐
    │   Tier: LOW  │   │ Tier: MEDIUM │       │  Tier: HIGH  │   │Tier: HIGH_ALT│
    │ Groq         │   │ Google       │       │ OpenAI       │   │ Anthropic    │
    │ Qwen 3.6 27B │   │ Gemini 3.6 Fl│       │ GPT-5.5 Pro  │   │ Claude Opus  │
    │ ($0.00 / 1M) │   │ ($0.00 / 1M) │       │ ($30.00 / 1M)│   │ ($5.00 / 1M) │
    └──────┬───────┘   └──────┬───────┘       └──────┬───────┘   └──────┬───────┘
           │                  │                      │                  │
           └──────────────────┼──────────────────────┴──────────────────┘
                              │
                      ...
                      │  - SSE Chunk Streaming / Full JSON     │
                      │  - Exponential Backoff Retry (Max 1)   │
                      │  - Strict 2048 Demo Token Enforcement  │
                      └───────────────────┬────────────────────┘
                                          │
                     ┌────────────────────┴────────────────────┐
                     │                                         │
                     ▼                                         ▼
      ┌─────────────────────────────┐           ┌─────────────────────────────┐
      │   Streaming Response (SSE)  │           │ Async Telemetry & Database  │
      │  - Standard OpenAI chunks   │           │  - Non-blocking Logger      │
      │  - Enriched routing metadata│           │  - Exact token delta        │
      │  - Actual vs Frontier costs │           │  - Postgres / Supabase ORM  │
      └─────────────────────────────┘           └─────────────────────────────┘
```
...
token-pilot/
├── packages/
│   └── classifier/             # Pure TypeScript library shared across backend & frontend
│       ├── src/
│       │   ├── feature-extractor.ts   # 12-signal extraction with negation awareness
│       │   ├── scoring-engine.ts      # Normalized linear scoring & confidence calculation
│       │   ├── types.ts               # Shared types across Gateway and Web UI
│       │   └── index.ts
│       └── package.json
├── src/                        # NestJS Gateway Backend (:3000)
│   ├── auth/                   # Master API key and Bearer auth guards
│   ├── classifier/             # Gateway wrapper supporting rules and LLM classifiers
│   ├── database/               # Drizzle ORM schema, migrations, and Supabase client
│   ├── logger/                 # Async request logging and exact dollar cost calculator
│   ├── providers/              # Adapters (Groq, Google, OpenAI, Anthropic, DeepSeek, Mock)
│   ├── rate-limiter/           # Sliding-window IP rate limiting guard
│   ├── router/                 # Core routing service, controller, and SSE streaming pipeline
│   ├── shared/                 # Cost registry, token estimators, and shared types
│   └── main.ts                 # Gateway bootstrap entry point
├── web/                        # Next.js 15 App Router Frontend (:3001)
│   ├── app/                    # Pages: Playground (/), Dashboard (/dashboard), Config (/config)
│   ├── components/
│   │   ├── playground/         # Prompt input, live routing visualization, cost delta comparison
│   │   ├── dashboard/          # Financial savings charts, tier distribution donut, latency bars
│   │   └── config/             # Real-time weight sliders, threshold tuning, provider health
│   └── lib/                    # Client state management, custom hooks, and API clients
├── eval/                       # Benchmark & Evaluation Harness
│   ├── prompts/eval-set.json   # 60 curated prompts partitioned across tiers
│   ├── scripts/                # Benchmark runner, LLM judge, and Pareto chart generator
│   └── results/                # Comparison logs, quality scores, and pareto.svg output
├── scripts/
│   └── setup-wizard.sh         # Interactive onboarding and environment configuration script
├── Dockerfile                  # Production multi-stage Docker build
├── drizzle.config.ts           # Drizzle schema and migration config
